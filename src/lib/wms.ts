import { EUMETSAT_LAYERS } from "@/config/sources";
...
export type ValidWmsRequest = { request: "GetMap" | "GetCapabilities"; layers?: (typeof EUMETSAT_LAYERS)[number]; bbox?: string; width?: string; height?: string; time?: string; format?: string; crs?: string };
const read = (input: URLSearchParams, key: string) => input.get(key) ?? input.get(key.toUpperCase());

export function validateWmsParams(input: URLSearchParams): ValidWmsRequest {
  ...
  if (!layer || !EUMETSAT_LAYERS.includes(layer as (typeof EUMETSAT_LAYERS)[number])) throw new Error("Unsupported EUMETSAT layer.");
  ...
  return { request: "GetMap", layers: layer as (typeof EUMETSAT_LAYERS)[number], bbox, width: String(width), height: String(height), crs, format, time };
}
